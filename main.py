...
def galaxies():
	...
	seedr = random.randint(100000, 999999)
	random.seed(seedr)
	os.chdir(oldir)
	grn = "#00FF00"
	org = "#FFA500"
	kltt = tk.Label(ryot, text='''галактика создается...''', font=("Arial", 17), fg=org, bg=bccv)
	kltt.pack(pady="20")
	...
	f"galaxy-{galx}.txt", "a"
		...
		kltt.destroy()
		klnt = tk.Label(ryot, text='''галактика создана!;>''', font=("Arial", 16), fg=grn, bg=bccv)
		klnt.pack(pady="25")
		kbt = tk.Label(ryot, text=f'''звезд: {zvezd}''', font=("Arial", 16),fg=grn,bg=bccv)
		kbt.pack(pady="5")
		kltrg = tk.Label(ryot, text=f'''нейтронных звезд: {neutronzvezd}''', font=("Arial", 16),fg=grn,bg=bccv)
		kltrg.pack(pady="5")
		kltrgn = tk.Label(ryot, text=f'''черных дыр: {blacabs}''', font=("Arial", 16),fg=grn,bg=bccv)
		kltrgn.pack(pady="5")
		kltrggn = tk.Label(ryot, text=f'''звездных скоплений: {zvezdscpl}''', font=("Arial", 16),fg=grn,bg=bccv)
		kltrggn.pack(pady="5")
		kltrggnn = tk.Label(ryot, text=f'''астероидов: {asteroids}''', font=("Arial", 16),fg=grn,bg=bccv)
		kltrggnn.pack(pady="5")
		kltrggkn = tk.Label(ryot, text=f'''планет: {planets}''', font=("Arial", 16),fg=grn,bg=bccv)
		kltrggkn.pack(pady="5")
		with open(
		f"stats-{galx}.txt", "a"
		)  as sts:
...
def ex():
	gb = "#00008B"
	gjk = tk.Label(ryot, text="пока, возвращайся еще!:D", font=("Arial", 16), fg=gb, bg=bccv)
	gjk.pack(pady="25")
	ryot.update()
	time.sleep(1.5)
	exit()
...
def thr():
	galxt = th.Thread(target=galaxies)
	galxt.daemon = True
	galxt.start()

def grmk(vlmm):
	vlmm = float(vlmm) / 100
	pygame.mixer.music.set_volume(vlmm)

ryot = tk.Tk()
ryot.title("Galaxies GUI")
ryot.geometry("900x850")
ryot.configure(bg="#0a0a2a")
vlmsk = tk.Scale(ryot, from_=0, to=100, orient="horizontal", label="Громкость", font=("Arial", 13), bg=bccv, command=grmk)
vlmsk.set(50)
vlmsk.place(x=610, y=160)
mnl = tk.Button(ryot, text="pause", font=("Arial", 19), bg=bccv, activebackground=wht, command=pause)
mnl.place(x=150, y=145)
mfl = tk.Button(ryot, text="unpause", font=("Arial", 17), bg=bccv, activebackground=wht, command=unpause)
mfl.place(x=150, y=240)
lnl = tk.Label(ryot, text='''Kosmo-Nyashka-Galaxies-GUI''', font=("Arial", 20), bg=bccv)
lnl.pack(pady="20")
snrtt = tk.Entry(ryot, font=("Arial", 16))
snrtt.pack(pady="70", padx="200")
sgt = tk.Button(ryot, text="создать галактику",font=("Arial", 16),activebackground=wht,bg=bccv,command=thr)
os.chdir(oldir)
sgt.pack(pady="15")
ext = tk.Button(ryot, text="Выход", font=("Arial", 16),bg=bccv, command=ex, activebackground=wht)
ext.pack(pady="25")
